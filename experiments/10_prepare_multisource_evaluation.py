"""Prepare and summarize multi-source contribution evaluation sheets."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Prepare multi-source evaluation sheets.")
    parser.add_argument(
        "--plan",
        type=Path,
        default=METRICS_DIR / "multisource_plan.csv",
        help="Multi-source plan CSV.",
    )
    parser.add_argument(
        "--main-eval",
        type=Path,
    ...
    parser.add_argument(
        "--sheet",
        type=Path,
        default=METRICS_DIR / "multisource_evaluation_sheet.csv",
        help="Output multi-source evaluation sheet.",
    )
    parser.add_argument(
        "--metrics",
        type=Path,
        default=METRICS_DIR / "multisource_metrics.csv",
        help="Output multi-source metrics summary.",
    )
    parser.add_argument(
        "--init-only",
        action="store_true",
...
def seed_rows(plan_rows: list[dict[str, str]], main_eval_rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    """Build a draft multi-source evaluation sheet."""
    ...
    for plan in plan_rows:
        ...
        if variant_id == "full":
            ...
            base["RDR_score"] = "3"
            base["EOR_score"] = "3"
            base["comments"] = "Draft penalty: no geological or forward state available."
        elif variant_id == "plc_geo":
            base["ICS_gas"] = "0"
            base["RDR_score"] = "4"
            base["EOR_score"] = "4"
            base["comments"] = "Draft penalty: geological state kept, gas-specific coverage removed."
...
def main() -> None:
    """Initialize and optionally summarize multi-source evaluation outputs."""
    args = parse_args()
    ...
    print(f"Wrote multi-source evaluation sheet: {args.sheet}")
    ...
    print(f"Wrote multi-source metrics: {args.metrics}")
