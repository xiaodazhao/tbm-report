...
- 项目一句话：本项目面向 TBM 施工日报自动生成，构建“多源证据约束生成 + ConstructionStateCell 施工状态单元 + Quality / Trace 可追溯校核”的研究框架。
...
- 指标边界：RAI 是 PLC 施工响应启发式关注度；GRS 是地质证据启发式关注度；GRCI 是已掘 cell 的地质-施工响应耦合启发式关注度，不是灾害概率，不是前方风险。没有 RAI 或没有 PLC response 的 cell 不计算正式 GRCI。当前没有灾害发生标签、异常原因标签或现场复核标签，因此不做监督学习式权重校准。

- 范围边界：`daily_plc_range` 是 PLC 实测推进范围；`daily_excavated_scope` 是 10m cell 对齐后的已掘复核范围，不是实际推进范围。默认 cell 为 10m，5m/10m 用于尺度敏感性分析，不默认使用 20m。系统设置 E14 检查，防止 LLM 把对齐范围误写为实际推进。

- 方法增强：Evidence Pack key cells 带有 `selection_score`、`selection_rank`、`selection_reason`；forward distance 分层为 0-10m、10-20m、20-30m；这些字段用于解释性导出和实验分析，不改变 RAI / GRS / GRCI 公式。
...
- 当前完成状态：方法主线已冻结；P0 Evidence Pack 约束生成、P1 Quality/Trace 修订与 E14、P2 Report Planner、91 天审计和批量运行、P3 sidecar 地质文本结构化、解释性字段与敏感性分析脚本均已完成；当前测试持续通过。
